...
<head>
    ...
    <title>Login - Sistem Kegiatan UNS</title>
    ...
    <style>
        ...
        :root {
          ...
          --uns-secondary: #008066; /* Hijau Sedang */
        }
        ...
        .form-signin {
            max-width: 450px; /* Lebar dinaikkan agar lebih kotak */
            
            /* PADDING INTERNAL CARD (MINIMAL VERTICAL) */
        ...
        /* Teks Error */
        .alert-danger {
             background-color: rgba(189, 21, 21, 0.3) !important;
             color: #fff;
...
<main class="form-signin w-100">
    <form method="POST" action="{{ route('login') }}">
        ...
        <h1 class="h3 mb-1 fw-bold">SELAMAT DATANG</h1>
        <p class="text-muted mb-4">Masuk ke Sistem Informasi Kegiatan Kampus UNS</p>
        ...
        <div class="form-floating mb-3">
            <input type="email" class="form-control @error('email') is-invalid @enderror" id="floatingEmail" placeholder="name@example.com" name="email" value="{{ old('email') }}" required autofocus autocomplete="username">
            <label for="floatingEmail">Email atau NIM</label>
        </div>
        ...
        {{-- CONTAINER LUPA PASSWORD & REGISTRASI (Dihapus: Ingat Saya) --}}
        <div class="d-flex justify-content-between align-items-center mb-5 mt-3">
            
            {{-- LUPA PASSWORD (PINDAH KE KIRI) --}}
            <div class="text-start">
                @if (Route::has('password.request'))
                    <a class="text-decoration-none text-link d-block" href="{{ route('password.request') }}">Lupa Password?</a>
                @endif
            </div>
            
            {{-- REGISTRASI AKUN (DI KANAN) --}}
            <div class="text-end">
                <a class="text-decoration-none text-link d-block" href="{{ route('register') }}">Registrasi Akun</a>
            </div>
        </div>
        
        {{-- TOMBOL SUBMIT --}}
        <button class="btn btn-lg btn-primary shadow-lg fw-bold w-100" type="submit">
            MASUK KE SISTEM
        </button>
